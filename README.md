...
### 4. Debugging GPU Hangs
If you encounter GPU hangs (especially when writing compute shaders), you can use the following tools and environment variables (on AMD GPUs):
*   Run the project with Vulkan validation and AMD's debug flags:
    ```bash
    RADV_DEBUG=hang,ds ./build/mooncrust examples/55_shallow_3d_renderer --validation
    ```
*   You can inspect the hang reports generated by `radv` using [UMR (User Mode Register Debugger)](https://gitlab.freedesktop.org/tomstdenis/umr).
