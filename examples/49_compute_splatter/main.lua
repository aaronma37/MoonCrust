...
function M.init()
    ...
    descriptors.update_buffer_set(device, bindless_set, 0, vk.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, large_splat_buf.handle, 0, large_splat_buf.size, 0)
    descriptors.update_buffer_set(device, bindless_set, 0, vk.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, large_count_buf.handle, 0, large_count_buf.size, 1)
    
    descriptors.update_storage_image_set(device, bindless_set, 2, vk.VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, storage_r.view, vk.VK_IMAGE_LAYOUT_GENERAL, 0)
    descriptors.update_storage_image_set(device, bindless_set, 2, vk.VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, storage_g.view, vk.VK_IMAGE_LAYOUT_GENERAL, 1)
    descriptors.update_storage_image_set(device, bindless_set, 2, vk.VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, storage_b.view, vk.VK_IMAGE_LAYOUT_GENERAL, 2)
    ...
    pipe_splat = pipeline.create_compute_pipeline(device, pipe_layout, shader.create_module(device, shader.compile_glsl(io.open("examples/49_compute_splatter/splat.comp"):read("*all"), vk.VK_SHADER_STAGE_COMPUTE_BIT)))
    pipe_resolve = pipeline.create_compute_pipeline(device, pipe_layout, shader.create_module(device, shader.compile_glsl(io.open("examples/49_compute_splatter/resolve.comp"):read("*all"), vk.VK_SHADER_STAGE_COMPUTE_BIT)))
    
    pipe_hybrid = pipeline.create_graphics_pipeline(device, pipe_layout, 
        shader.create_module(device, shader.compile_glsl(io.open("examples/49_compute_splatter/hybrid.vert"):read("*all"), vk.VK_SHADER_STAGE_VERTEX_BIT)), 
        shader.create_module(device, shader.compile_glsl(io.open("examples/49_compute_splatter/hybrid.frag"):read("*all"), vk.VK_SHADER_STAGE_FRAGMENT_BIT)), 
        { topology = vk.VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP, additive = true, color_formats = {vk.VK_FORMAT_R32G32B32A32_SFLOAT} })
...
function M.update()
    ...
    input.tick(); M.current_time = M.current_time + dt
    ...
    local speed = 5.0 * dt; if input.key_down(input.SCANCODE_LSHIFT) then speed = 15.0 * dt end
    ...
    local sub_range = ffi.new("VkImageSubresourceRange", {aspectMask=vk.VK_IMAGE_ASPECT_COLOR_BIT, levelCount=1, layerCount=1})
    local clear_bars = ffi.new("VkImageMemoryBarrier[3]")
    local clear_imgs = {storage_r.handle, storage_g.handle, storage_b.handle}
    for i=0,2 do clear_bars[i].sType, clear_bars[i].oldLayout, clear_bars[i].newLayout, clear_bars[i].image, clear_bars[i].subresourceRange, clear_bars[i].dstAccessMask = vk.VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, vk.VK_IMAGE_LAYOUT_UNDEFINED, vk.VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, clear_imgs[i+1], sub_range, vk.VK_ACCESS_TRANSFER_WRITE_BIT end
    vk.vkCmdPipelineBarrier(cb, vk.VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, vk.VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nil, 0, nil, 3, clear_bars)
    for i=0,2 do vk.vkCmdClearColorImage(cb, clear_imgs[i+1], vk.VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, color_zero, 1, sub_range) end
    vk.vkCmdFillBuffer(cb, large_count_buf.handle, 0, 4, 0) -- Reset large splat counter

    local b_sync = ffi.new("VkBufferMemoryBarrier[1]", {{ sType=vk.VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER, srcAccessMask=vk.VK_ACCESS_TRANSFER_WRITE_BIT, dstAccessMask=vk.VK_ACCESS_SHADER_READ_BIT + vk.VK_ACCESS_SHADER_WRITE_BIT, buffer=large_count_buf.handle, size=4 }})
    ...
    for i=0,2 do clear_bars[i].oldLayout, clear_bars[i].newLayout, clear_bars[i].srcAccessMask, clear_bars[i].dstAccessMask = vk.VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, vk.VK_IMAGE_LAYOUT_GENERAL, vk.VK_ACCESS_TRANSFER_WRITE_BIT, vk.VK_ACCESS_SHADER_READ_BIT + vk.VK_ACCESS_SHADER_WRITE_BIT end
    vk.vkCmdPipelineBarrier(cb, vk.VK_PIPELINE_STAGE_TRANSFER_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nil, 0, nil, 3, clear_bars)

    graph:reset()
    
    -- Pass 1: Splatting (Assigns large splats to buffer, atomics small splats)
    graph:add_pass("Splat", function(c)
        ...
        local pc = ffi.new("SplatPC", { view = view, proj = proj, focal = focal, large_s_id = 0, large_c_id = 1, count = GAUSSIAN_COUNT, screen_w = sw.extent.width, screen_h = sw.extent.height, time = M.current_time })
        vk.vkCmdPushConstants(c, pipe_layout, vk.VK_SHADER_STAGE_ALL, 0, 256, pc)
        vk.vkCmdDispatch(c, math.ceil(GAUSSIAN_COUNT / 256), 1, 1)
    end):using(graph.r, vk.VK_ACCESS_SHADER_WRITE_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk.VK_IMAGE_LAYOUT_GENERAL)
       :using(graph.g, vk.VK_ACCESS_SHADER_WRITE_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk.VK_IMAGE_LAYOUT_GENERAL)
       :using(graph.b, vk.VK_ACCESS_SHADER_WRITE_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk.VK_IMAGE_LAYOUT_GENERAL)
       :using(graph.l_splats, vk.VK_ACCESS_SHADER_WRITE_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT)
       :using(graph.l_count, vk.VK_ACCESS_SHADER_WRITE_BIT + vk.VK_ACCESS_SHADER_READ_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT)

    -- Pass 2: Resolve and Render Hybrid Large Splats
    graph:add_pass("Resolve", function(c)
    ...
    end):using(graph.r, vk.VK_ACCESS_SHADER_READ_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk.VK_IMAGE_LAYOUT_GENERAL)
       :using(graph.g, vk.VK_ACCESS_SHADER_READ_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk.VK_IMAGE_LAYOUT_GENERAL)
       :using(graph.b, vk.VK_ACCESS_SHADER_READ_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk.VK_IMAGE_LAYOUT_GENERAL)
       :using(graph.storage, vk.VK_ACCESS_SHADER_WRITE_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk.VK_IMAGE_LAYOUT_GENERAL)

    graph:add_pass("LargeSplats", function(c)
        local color_attach = ffi.new("VkRenderingAttachmentInfo[1]")
        color_attach[0].sType, color_attach[0].imageView, color_attach[0].imageLayout = vk.VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO, storage_img.view, vk.VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL
        color_attach[0].loadOp, color_attach[0].storeOp = vk.VK_ATTACHMENT_LOAD_OP_LOAD, vk.VK_ATTACHMENT_STORE_OP_STORE
    ...
    end):using(graph.storage, vk.VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, vk.VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, vk.VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL)
    ...
    end):using(graph.storage, vk.VK_ACCESS_TRANSFER_READ_BIT, vk.VK_PIPELINE_STAGE_TRANSFER_BIT, vk.VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL)
       :using(sw_res[idx], vk.VK_ACCESS_TRANSFER_WRITE_BIT, vk.VK_PIPELINE_STAGE_TRANSFER_BIT, vk.VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)
