...
function M.create_params(num_bones, rings_per_bone, segments)
	...
	local joints = {
		...
		mixamorig_HeadTop_End = { r = 0.0, oval = 0.0 },
		virtual_Forehead = { r = 0.9, oval = 0.2, back_shift = 0.3 },
		virtual_Crown = { r = 0.9, oval = 0.2, taper = 1.0, back_shift = 0.4 },
		virtual_Jaw = { r = 0.5, oval = 0.2 },
		virtual_Chin = { r = 0.05, oval = 0.4, taper = 1.0 },
	...
	local function get_joint(name)
		return joints[name] or { r = 0.4, oval = 0.2, taper = 0.0, back_shift = 0.0 }
	end
	...
	for b = 0, num_bones - 1 do
		...
		if
			seg.parent_name == "mixamorig_Spine2" and (seg.child_name:find("Shoulder") or seg.child_name:find("Arm"))
		then
			s_r = 0.5
		end
		...
		for r = 0, rings_per_bone - 1 do
			...
			local cur_back = (start_j.back_shift or 0.0) * (1.0 - rt) + (end_j.back_shift or 0.0) * rt

			p.coeffs[0] = cur_r
			p.coeffs[1] = 0 -- x trans
			p.coeffs[2] = -cur_back * cur_r -- z trans (BACKWARD SHIFT)
			p.coeffs[3] = cur_oval * cur_r
			p.coeffs[7] = cur_taper

			-- Keep the s_push logic separate if needed
			if s_push ~= 0 then
				p.coeffs[2] = p.coeffs[2] + (s_push * (1.0 - rt) + e_push * rt)
			end
		end
	end
		...
		for prev_idx = 0, num_bones - 1 do
			...
			if prev.end_pos[4] == curr.start_pos[4] then
				local function is_torso(name)
					return name:find("Spine")
						or name:find("Hips")
						or name:find("Neck")
						or name:find("Head")
						or name:find("virtual")
				end
				if is_torso(prev.child_name) == is_torso(curr.child_name) then
					...
					for i = 0, 7 do
						c_first.coeffs[i] = p_last.coeffs[i]
					end
				end
			end
		end
