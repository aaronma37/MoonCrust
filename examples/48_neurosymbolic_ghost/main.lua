...
local M = { cam_pos = {0, 2, 8}, cam_rot = {0, 0}, current_time = 0 }

-- CONFIG
local GAUSSIAN_COUNT = 131072 -- 2^17
-- MLP 3 -> 16 -> 9
...
function M.init()
    print("Example 48: NEURO-SYMBOLIC GHOST GENERATION")
    ...
    g_buffer = mc.buffer(64 * GAUSSIAN_COUNT, "storage")
    p_buffer = mc.buffer(44 * GAUSSIAN_COUNT, "storage")
    s_buffer = mc.buffer(8 * GAUSSIAN_COUNT, "storage")
    ...
    pipe_gen = pipeline.create_compute_pipeline(device, pipe_layout, shader.create_module(device, shader.compile_glsl(io.open("examples/48_neurosymbolic_ghost/generator.comp"):read("*all"), vk.VK_SHADER_STAGE_COMPUTE_BIT)))
    pipe_project = pipeline.create_compute_pipeline(device, pipe_layout, shader.create_module(device, shader.compile_glsl(io.open("examples/48_neurosymbolic_ghost/project.comp"):read("*all"), vk.VK_SHADER_STAGE_COMPUTE_BIT)))
    pipe_sort = pipeline.create_compute_pipeline(device, pipe_layout, shader.create_module(device, shader.compile_glsl(io.open("examples/48_neurosymbolic_ghost/sort.comp"):read("*all"), vk.VK_SHADER_STAGE_COMPUTE_BIT)))
    pipe_render = pipeline.create_graphics_pipeline(device, pipe_layout, 
        shader.create_module(device, shader.compile_glsl(io.open("examples/48_neurosymbolic_ghost/render.vert"):read("*all"), vk.VK_SHADER_STAGE_VERTEX_BIT)), 
        shader.create_module(device, shader.compile_glsl(io.open("examples/48_neurosymbolic_ghost/render.frag"):read("*all"), vk.VK_SHADER_STAGE_FRAGMENT_BIT)), 
...
function M.update()
    ...
    graph:add_pass("Render", function(c)
        ...
        color_attach[0].loadOp, color_attach[0].storeOp, color_attach[0].clearValue.color.float32 = vk.VK_ATTACHMENT_LOAD_OP_CLEAR, vk.VK_ATTACHMENT_STORE_OP_STORE, {0.02,0.02,0.05,1}
