...
function M.init()
    print("MoonCrust: 1M Particles with Auto-Sync Render Graph")
    ...
    ffi.cdef[[
        typedef struct Particle { float px, py, vx, vy; } Particle;
    ...
    -- Allocation
    local host_heap = heap.new(physical_device, device, heap.find_memory_type(physical_device, 0xFFFFFFFF, bit.bor(vk.VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, vk.VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)), 64 * 1024 * 1024)
    local device_heap = heap.new(physical_device, device, heap.find_memory_type(physical_device, 0xFFFFFFFF, vk.VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT), 128 * 1024 * 1024)
    ...
    for i = 0, PARTICLE_COUNT - 1 do initial_data[i].px = (math.random()*2)-1; initial_data[i].py = (math.random()*2)-1 end
    ...
    local glow_img = image.create_2d(device, TW, TH, vk.VK_FORMAT_R8G8B8A8_UNORM, bit.bor(vk.VK_IMAGE_USAGE_SAMPLED_BIT, vk.VK_IMAGE_USAGE_TRANSFER_DST_BIT))
    ...
    local glow_view = image.create_view(device, glow_img, vk.VK_FORMAT_R8G8B8A8_UNORM)
...
function M.update()
    ...
    graph:add_pass("Physics", function(cb)
        ...
        vk.vkCmdPushConstants(cb, pipe_layout, stages, 0, 16, pc)
    ...
    graph:add_pass("Render", function(cb)
        ...
        color_attach[0].sType, color_attach[0].imageView, color_attach[0].imageLayout = vk.VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO, view, vk.VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL
    ...
    -- PASS 3: Presentation Transition
    graph:add_pass("PresentPrep", function(cb) end)
       :using(g_swImages[img_idx], 0, vk.VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, vk.VK_IMAGE_LAYOUT_PRESENT_SRC_KHR)

    graph:execute(cb)
    vk.vkEndCommandBuffer(cb)

    -- Submit
    local pCBs, pWaitSems, pWaitStages = ffi.new("VkCommandBuffer[1]", {cb}), ffi.new("VkSemaphore[1]", {image_available_sem}), ffi.new("VkPipelineStageFlags[1]", {vk.VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT})
    local render_finished_sem = sw.semaphores[img_idx]
    local pSignalSems = ffi.new("VkSemaphore[1]", {render_finished_sem})
    vk.vkQueueSubmit(queue, 1, ffi.new("VkSubmitInfo", { sType = vk.VK_STRUCTURE_TYPE_SUBMIT_INFO, waitSemaphoreCount = 1, pWaitSemaphores = pWaitSems, pWaitDstStageMask = pWaitStages, commandBufferCount = 1, pCommandBuffers = pCBs, signalSemaphoreCount = 1, pSignalSemaphores = pSignalSems }), frame_fence)
