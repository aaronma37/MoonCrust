...
local sets, cbs, submit_infos, pFenceArr

function M.init()
    print("Example 06: Visual 1M Particle Simulation - Static Module Boot")
    ...
    for i = 0, PARTICLE_COUNT - 1 do
        initial_data[i].px = 0.0
        initial_data[i].py = 0.0
    ...
    local dpool = descriptors.create_pool(device, {{ type = vk.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, count = 1 }}, 1)
    ...
    for i=0, sw.image_count-1 do
        ...
        vk.vkBeginCommandBuffer(cb, ffi.new("VkCommandBufferBeginInfo", {
        ...
        local pSetsArr = ffi.new("VkDescriptorSet[1]", {sets[1]})
        ...
        vk.vkCmdPushConstants(cb, pipe_layout, vk.VK_SHADER_STAGE_COMPUTE_BIT, 0, 4, dt)
        ...
        local render_info = ffi.new("VkRenderingInfo", {
            ...
            renderArea = { offset = {0, 0}, extent = sw.extent },
            layerCount = 1,
        ...
        local vp = ffi.new("VkViewport", { x=0, y=0, width=sw.extent.width, height=sw.extent.height, minDepth=0, maxDepth=1 })
        vk.vkCmdSetViewport(cb, 0, 1, vp)
        local sc = ffi.new("VkRect2D", { offset={0,0}, extent=sw.extent })
        vk.vkCmdSetScissor(cb, 0, 1, sc)
        ...
        local pSignalSems = ffi.new("VkSemaphore[1]", {sw.semaphores[i]})
        local sub = ffi.new("VkSubmitInfo", {
            sType = vk.VK_STRUCTURE_TYPE_SUBMIT_INFO,
            waitSemaphoreCount = 1,
            pWaitSemaphores = pWaitSems,
            pWaitDstStageMask = pWaitStages,
            commandBufferCount = 1,
            pCommandBuffers = pCBs,
            signalSemaphoreCount = 1,
            pSignalSemaphores = pSignalSems
        })
        -- Store everything in a module-level table to prevent GC
        submit_infos[i+1] = { info = sub, pCBs = pCBs, pSignalSems = pSignalSems }
    end

    print("Hybrid example initialized.")
end
