...
function M.init()
    print("Example 09: FLUID SPH (Full Physics)")
    ...
    local pBuf = ffi.new("VkBuffer[1]"); vk.vkCreateBuffer(device, ffi.new("VkBufferCreateInfo", { sType = vk.VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO, size = p_size, usage = bit.bor(vk.VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, vk.VK_BUFFER_USAGE_TRANSFER_DST_BIT) }), nil, pBuf)
    local sBuf = ffi.new("VkBuffer[1]"); vk.vkCreateBuffer(device, ffi.new("VkBufferCreateInfo", { sType = vk.VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO, size = s_size, usage = vk.VK_BUFFER_USAGE_STORAGE_BUFFER_BIT }), nil, sBuf)
    local p_alloc = device_heap:malloc(p_size); vk.vkBindBufferMemory(device, pBuf[0], p_alloc.memory, p_alloc.offset)
    local s_alloc = device_heap:malloc(s_size); vk.vkBindBufferMemory(device, sBuf[0], s_alloc.memory, s_alloc.offset)
    local particle_buffer, sort_buffer = pBuf[0], sBuf[0]
    ...
    staging.new(physical_device, device, host_heap, p_size + 1024):upload_buffer(particle_buffer, initial_data, 0, queue, graphics_family)
    ...
    descriptors.update_buffer_set(device, bindless_set, 0, vk.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, particle_buffer, 0, p_size, 0)
    descriptors.update_buffer_set(device, bindless_set, 0, vk.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, sort_buffer, 0, s_size, 1)

    -- Pipelines
    M.layout = pipeline.create_layout(device, {bl_layout}, ffi.new("VkPushConstantRange[1]", {{ stageFlags = vk.VK_SHADER_STAGE_ALL, offset = 0, size = 32 }}))
    ...
    g_pBuf = graph:register_resource("Particles", render_graph.TYPE_BUFFER, particle_buffer)
    g_sBuf = graph:register_resource("Sort", render_graph.TYPE_BUFFER, sort_buffer)
...
function M.update()
    ...
    graph:add_pass("Hash", function(c)
        ...
        vk.vkCmdPushConstants(c, M.layout, vk.VK_SHADER_STAGE_ALL, 0, 16, ffi.new("HashPC", {0, 1, PARTICLE_COUNT, GRID_SIZE}))
    ...
    for stage = 0, num_stages - 1 do for pass = stage, 0, -1 do
        graph:add_pass("Sort", function(c)
            ...
            vk.vkCmdBindDescriptorSets(c, vk.VK_PIPELINE_BIND_POINT_COMPUTE, M.layout, 0, 1, pSetsArr, 0, nil)
            vk.vkCmdPushConstants(c, M.layout, vk.VK_SHADER_STAGE_ALL, 0, 12, ffi.new("SortPC", {1, bit.lshift(1, pass), bit.lshift(1, stage + 1)}))
            vk.vkCmdDispatch(c, math.ceil(PARTICLE_COUNT / 256), 1, 1)
    ...
    graph:add_pass("Density", function(c)
        ...
        vk.vkCmdBindDescriptorSets(c, vk.VK_PIPELINE_BIND_POINT_COMPUTE, M.layout, 0, 1, pSetsArr, 0, nil)
        vk.vkCmdPushConstants(c, M.layout, vk.VK_SHADER_STAGE_ALL, 0, 20, ffi.new("PhysicsPC", {0, 1, PARTICLE_COUNT, 0.016, GRID_SIZE}))
        vk.vkCmdDispatch(c, math.ceil(PARTICLE_COUNT / 256), 1, 1)
    ...
    graph:add_pass("Render", function(c)
        ...
        vk.vkCmdPushConstants(c, M.layout, vk.VK_SHADER_STAGE_ALL, 0, 8, ffi.new("RenderPC", {0.016, 0}))
