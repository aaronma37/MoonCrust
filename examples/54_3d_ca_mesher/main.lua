...
function M.init()
    ...
    -- 3. Pipelines
    pipe_sim = mc.gpu.compute_pipeline("examples/54_3d_ca_mesher/sim.comp", 16)
    pipe_mesh = mc.gpu.compute_pipeline("examples/54_3d_ca_mesher/mesh.comp", 20)
    pipe_gen = mc.gpu.compute_pipeline("examples/54_3d_ca_mesher/gen.comp", 12)
    ...
    local v_mod = shader.create_module(device, shader.compile_glsl(io.open("examples/54_3d_ca_mesher/render.vert"):read("*all"), vk.VK_SHADER_STAGE_VERTEX_BIT))
    local f_mod = shader.create_module(device, shader.compile_glsl(io.open("examples/54_3d_ca_mesher/render.frag"):read("*all"), vk.VK_SHADER_STAGE_FRAGMENT_BIT))
    pipe_render = pipeline.create_graphics_pipeline(device, render_layout, v_mod, f_mod, { depth_test = true })
...
function M.update()
    ...
    if input.key_down(input.SCANCODE_S) then M.cam_pos[3] = M.cam_pos[3] - 1.5 end
    if input.key_down(input.SCANCODE_A) then M.cam_pos[1] = M.cam_pos[1] - 1.5 end
    if input.key_down(input.SCANCODE_D) then M.cam_pos[1] = M.cam_pos[1] + 1.5 end

    local aspect = sw.extent.width / sw.extent.height
    local proj = mc.mat4_perspective(mc.rad(45), aspect, 0.1, 1000.0)
    local view = mc.mat4_look_at({M.cam_pos[1], M.cam_pos[2], M.cam_pos[3]}, {64,64,64}, {0,1,0})
    ...
    rg:add_pass("CA_Sim", function(cb)
        ...
        vk.vkCmdPushConstants(cb, pipe_sim.layout, vk.VK_SHADER_STAGE_COMPUTE_BIT, 0, 16, ffi.new("uint32_t[4]", {vol_in_idx, vol_out_idx, 0, M.grid_res}))
        vk.vkCmdDispatch(cb, M.grid_res/16, M.grid_res/16, M.grid_res)
    ...
    rg:add_pass("Mesher", function(cb)
        ...
        vk.vkCmdPushConstants(cb, pipe_mesh.layout, vk.VK_SHADER_STAGE_COMPUTE_BIT, 0, 20, ffi.new("uint32_t[5]", {vol_out_idx, 0, 1, 2, M.grid_res}))
        vk.vkCmdDispatch(cb, M.grid_res/16, M.grid_res/16, M.grid_res/16)
    end):using(vol_out_res, vk.VK_ACCESS_SHADER_READ_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk.VK_IMAGE_LAYOUT_GENERAL)
