...
local constants = require("examples.32_cellular_automata.constants")
...
local function load_glsl(path)
    local f = io.open("examples/32_cellular_automata/shaders/" .. path, "r")
...
local function build_physics_shader()
    ...
    local sa = load_glsl("physics_sa.glsl")
    ...
    return string.format([[
        ...
        void main() {
            ...
            self = all_buffers[pc.in_buf_idx].cells[pos.y * pc.world_w + pos.x];
            next = self;

            // 1. RUN SA PHYSICS (Movement/Equilibrium)
            main_sa();
            
            // 2. RUN BOTANY (Growth/Expansion)
            // Botany overrides 'next' because growth is a dominant life process.
            run_botany();

            // 3. BRUSH LOGIC
            if (pc.brush_material != 0) {
                vec2 world_mouse = vec2(pc.cam_x + pc.mouse_x * view_w, pc.cam_y + pc.mouse_y * view_h);
                uint id = pc.brush_material;
                bool in_brush = false;
                if (id == 7) {
                    if (pos == ivec2(world_mouse)) in_brush = true;
                } else {
                    if (distance(vec2(pos), world_mouse) < float(pc.brush_size)) in_brush = true;
                }

                if (in_brush) {
                    uint flags = 0;
                    uint life = 0;
                    if (id == 6 || id == 10 || id == 11 || id == 14) flags = 1; // Solid/Organic
                    if (id == 12) { life = (15u << 4) | 15u; flags = 1; } // Tip
                    if (id == 7) { life = 0; flags = 0; } // Heart (Dynamic)
                    
                    next.data0 = pack_data0(id, 128, 0, 0);
                    next.data1 = pack_data1(100, life, 0, 0, flags);
                }
            }
            
            all_buffers[pc.out_buf_idx].cells[pos.y * pc.world_w + pos.x] = next;
        }
    ]], common, sa, botany)
end

function M.init()
    print("Example 32: HAMILTONIAN SA PHYSICS")
...
function M.update()
    ...
    if input.key_down(input.SCANCODE_6) then M.brush_material = 7 end
