...
local cam_rot = { 0, 0 }
local cam_dist = 3.5
local cam_target = { 0, 0.5, 0 }

-- SDL3 Button Masks
local SDL_BUTTON_LMASK = bit.lshift(1, 0)
local SDL_BUTTON_MMASK = bit.lshift(1, 1)
local SDL_BUTTON_RMASK = bit.lshift(1, 2)
...
function M.init()
	...
	-- local loadout = {
	--     helmet = "bucket",
	--     shoulders = "plate_pauldrons",
	--     arms = "plate_bracers",
	--     right_hand = "sword"
	-- }
    local loadout = {}
    local equipped_sdfs = generator.equip_character(skeleton_tree, loadout, 0)
	sdf_count = #equipped_sdfs

	local cape_p, cape_c = generator.generate_dynamic_assets(loadout, bone_map)
	...
	for i, s in ipairs(equipped_sdfs) do
		...
		d.bone_index, d.primitive_type, d.id, d.color_packed =
			bone_map[s.bone], primitive_map[s.type] or 0, s.id, s.color
		for j = 1, 4 do
			d.params[j - 1] = s.params[j]
		end
	...
	for i, p in ipairs(cape_p) do
		...
		if p.bone_index >= 0 then
			...
			for j = 0, 15 do
				inv_m.m[j] = bone_data[p.bone_index].inv_world_matrix[j]
			end
	...
	descriptors.update_buffer_set(
		...
		0,
		vk.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
		constraint_buffer.handle,
		0,
		constraint_buffer.size,
		3
	)
	descriptors.update_buffer_set(
		device,
	...
	local render_pc =
		ffi.new("VkPushConstantRange[1]", { { stageFlags = stages, offset = 0, size = ffi.sizeof("PushConstants") } })
	local phys_pc = ffi.new(
		"VkPushConstantRange[1]",
		{ { stageFlags = stages, offset = 0, size = ffi.sizeof("PhysicsPushConstants") } }
	)
	local bin_pc = ffi.new(
		"VkPushConstantRange[1]",
		{ { stageFlags = stages, offset = 0, size = ffi.sizeof("BinningPushConstants") } }
	)
	...
	render_pipe = pipeline.create_compute_pipeline(
		device,
		pipe_layout,
		shader.create_module(device, shader.compile_glsl(render_src, vk.VK_SHADER_STAGE_COMPUTE_BIT))
	)
	physics_pipe = pipeline.create_compute_pipeline(
		device,
		phys_layout,
		shader.create_module(device, shader.compile_glsl(physics_src, vk.VK_SHADER_STAGE_COMPUTE_BIT))
	)
	binning_pipe = pipeline.create_compute_pipeline(
		device,
		bin_layout,
		shader.create_module(device, shader.compile_glsl(binning_src, vk.VK_SHADER_STAGE_COMPUTE_BIT))
	)
	...
	for i = 0, sw.image_count - 1 do
		g_swImages[i] = graph:register_resource("SwapchainImage_" .. i, render_graph.TYPE_IMAGE, sw.images[i])
	end
...
function M.update()
	...
	vk.vkWaitForFences(
		device,
		1,
		ffi.new("VkFence[1]", { frame_fences[current_frame] }),
		vk.VK_TRUE,
	...
	-- Update SDF buffer for breathing/dynamic geometry
	local dynamic_sdfs = generator.equip_character(skeleton_tree, loadout, current_time)
	for i, s in ipairs(dynamic_sdfs) do
		local d = sdf_data[i - 1]
		d.params[0] = s.params[1] -- Update radius (breathing)
	end
	...
	local pc_bin = ffi.new(
		"BinningPushConstants",
		{ particle_count = particle_count, cell_size = 0.25, insert_radius = 0.18, padding = 0 }
	)
	local pc_render = ffi.new("PushConstants")
	pc_render.cam_pos = { cx, cy, cz, 1 }
	pc_render.cam_dir = { cam_target[1] - cx, cam_target[2] - cy, cam_target[3] - cz, 0 }
	pc_render.cam_up = { 0, 1, 0, 0 }
	pc_render.time, pc_render.bone_count, pc_render.sdf_count, pc_render.particle_count, pc_render.out_tex_id =
		current_time, #skeleton_order, sdf_count, particle_count, 0

	graph:reset()

	graph
		:add_pass("Physics", function(c)
			...
			vk.vkCmdPushConstants(
				...
				0,
				ffi.sizeof("PhysicsPushConstants"),
				pc_phys
			)
			vk.vkCmdDispatch(c, math.ceil(particle_count / 256), 1, 1)
		end)
		:using(g_pBuffer, vk.VK_ACCESS_SHADER_WRITE_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT)

	graph
		:add_pass("ClearGrid", function(c)
			vk.vkCmdFillBuffer(c, grid_buffer.handle, 0, grid_buffer.size, 0)
		end)
		:using(g_gridBuffer, vk.VK_ACCESS_TRANSFER_WRITE_BIT, vk.VK_PIPELINE_STAGE_TRANSFER_BIT)
	...
	graph
		:add_pass("Binning", function(c)
			...
			vk.vkCmdPushConstants(
				...
				vk.VK_SHADER_STAGE_COMPUTE_BIT,
				0,
				ffi.sizeof("BinningPushConstants"),
				pc_bin
			)
			vk.vkCmdDispatch(c, math.ceil(particle_count / 256), 1, 1)
		end)
		:using(g_gridBuffer, vk.VK_ACCESS_SHADER_WRITE_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT)
		:using(g_pBuffer, vk.VK_ACCESS_SHADER_READ_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT)

	graph
		:add_pass("Raymarch", function(c)
	...
	graph
		:add_pass("BlitToSwapchain", function(c)
			local blit = ffi.new("VkImageBlit[1]")
			blit[0].srcSubresource, blit[0].dstSubresource =
				{ aspectMask = vk.VK_IMAGE_ASPECT_COLOR_BIT, layerCount = 1 },
				{ aspectMask = vk.VK_IMAGE_ASPECT_COLOR_BIT, layerCount = 1 }
			blit[0].srcOffsets[1], blit[0].dstOffsets[1] =
				{ x = LOW_RES_W, y = LOW_RES_H, z = 1 }, { x = sw.extent.width, y = sw.extent.height, z = 1 }
			vk.vkCmdBlitImage(
				c,
				render_img.handle,
	...
	graph
		:add_pass("PresentPrep", function(c) end)
		:using(g_swImages[img_idx], 0, vk.VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, vk.VK_IMAGE_LAYOUT_PRESENT_SRC_KHR)
	...
	vk.vkQueueSubmit(
		...
		ffi.new(
			"VkSubmitInfo",
			{
				...
				pWaitSemaphores = ffi.new("VkSemaphore[1]", { image_available_sems[current_frame] }),
				...
				pSignalSemaphores = ffi.new("VkSemaphore[1]", { render_finished_sems[current_frame] }),
			}
		),
