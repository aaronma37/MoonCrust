...
function M.init()
    print("Example 51: Neurosymbolic SDF Renderer (Phase 3: Grid Binning + Optimization)")
    ...
    local grouped_sdfs = generator.equip_character(skeleton_tree, loadout, 0)
    ...
    -- Fill SDF Buffer contiguously per bone
    ...
    for i, name in ipairs(skeleton_order) do
        ...
        if group then
            ...
            bone_data[b_idx].bounding_radius = 0
            bone_data[b_idx].sdf_offset = 0
            bone_data[b_idx].sdf_count = 0
        end
    end
    ...
    grid_buffer = mc.gpu.buffer(64 * GRID_CELLS, "storage", nil, false) 
...
function M.update()
    ...
    local dynamic_grouped = generator.equip_character(skeleton_tree, loadout, current_time)
    for i, name in ipairs(skeleton_order) do
        local group = dynamic_grouped[name]
        if group then
            local b_idx = i - 1
            local offset = bone_data[b_idx].sdf_offset
            for j, s in ipairs(group.sdfs) do
                sdf_data[offset + j - 1].params[0] = s.params[1]
            end
        end
    end
