...
local NUM_ITEMS = 40
local MAX_NODES = 524288 -- 0.5M
local CAPACITY = 200.0
...
function M.generate_problem()
    ...
    for i = 0, NUM_ITEMS - 1 do
        items[i].val = math.random(10, 100)
        items[i].weight = math.random(5, 50)
    end
    -- Sort by efficiency
    local item_list = {}
    ...
    stats[0].count = 1; stats[0].best_uint = 0
...
function M.init()
    ...
    ffi.cdef[[
        ...
        typedef struct PushConstants {
            float dt, time;
            uint32_t max_nodes, num_items;
            float capacity;
            uint32_t start_idx, num_to_process, mode;
        } PushConstants;
    ]]
    ...
    g_items = graph:register_resource("Items", render_graph.TYPE_BUFFER, buf_items.handle)
    g_nodes = graph:register_resource("TreeNodes", render_graph.TYPE_BUFFER, buf_nodes.handle)
    g_stats = graph:register_resource("Stats", render_graph.TYPE_BUFFER, buf_stats.handle)
    ...
    for i=0, M.MAX_FRAMES_IN_FLIGHT-1 do
        ...
        local pS1 = ffi.new("VkSemaphore[1]")
        vk.vkCreateSemaphore(device, sem_info, nil, pS1); image_available_sems[i] = pS1[0]
        local pS2 = ffi.new("VkSemaphore[1]")
        vk.vkCreateSemaphore(device, sem_info, nil, pS2); render_finished_sems[i] = pS2[0]
    end
...
function M.update()
    ...
    local dt = (ticks - M.last_ticks) / 1000.0
    M.last_ticks = ticks
    ...
    -- Branching Strategy: process 500 nodes per frame for a fast bloom
    ...
    if M.nodes_done < MAX_NODES and nodes_to_eval > 0 then
        graph:add_pass("BranchAndBound", function(c)
            ...
            pc.start_idx = M.nodes_done
            pc.num_to_process = nodes_to_eval
            pc.mode = 0
            vk.vkCmdPushConstants(c, pipe_layout, vk.VK_SHADER_STAGE_ALL, 0, 32, pc)
            vk.vkCmdDispatch(c, math.ceil(nodes_to_eval / 64), 1, 1)
            M.nodes_done = M.nodes_done + nodes_to_eval
        end):using(M.g_items, vk.VK_ACCESS_SHADER_READ_BIT, vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT)
           :using(M.g_nodes, bit.bor(vk.VK_ACCESS_SHADER_READ_BIT, vk.VK_ACCESS_SHADER_WRITE_BIT), vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT)
           :using(M.g_stats, bit.bor(vk.VK_ACCESS_SHADER_READ_BIT, vk.VK_ACCESS_SHADER_WRITE_BIT), vk.VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT)
    end
    ...
    graph:add_pass("Render", function(c)
        ...
        color_attach[0].clearValue.color.float32[0]=0.01; color_attach[0].clearValue.color.float32[1]=0.01; color_attach[0].clearValue.color.float32[2]=0.02; color_attach[0].clearValue.color.float32[3]=1.0
        ...
        vk.vkCmdDraw(c, draw_count * 2, 1, 0, 0)
        pc.mode = 2
        vk.vkCmdBindPipeline(c, vk.VK_PIPELINE_BIND_POINT_GRAPHICS, graphics_pipe)
        vk.vkCmdPushConstants(c, pipe_layout, vk.VK_SHADER_STAGE_ALL, 0, 32, pc)
        vk.vkCmdDraw(c, draw_count, 1, 0, 0)
        vk.vkCmdEndRendering(c)
    end):using(M.g_nodes, vk.VK_ACCESS_SHADER_READ_BIT, vk.VK_PIPELINE_STAGE_VERTEX_SHADER_BIT)
    ...
    vk.vkQueueSubmit(queue, 1, ffi.new("VkSubmitInfo", { 
        ...
        waitSemaphoreCount = 1, pWaitSemaphores = ffi.new("VkSemaphore[1]", {image_available_sems[M.current_frame]}), 
        ...
        signalSemaphoreCount = 1, pSignalSemaphores = ffi.new("VkSemaphore[1]", {render_finished_sems[M.current_frame]}) 
    }), frame_fences[M.current_frame])
    
    sw:present(queue, img_idx, render_finished_sems[M.current_frame])
