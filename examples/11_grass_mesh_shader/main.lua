...
local math_utils = require("examples.10_moo_graph_search.math") -- Reuse math
...
function M.init()
    print("Example 11: INFINITE GRASS (Mesh Shaders)")
    ...
    -- 1. Bindless (Empty but required by layout)
...
function M.update()
    ...
    local view = math_utils.look_at({math.cos(M.angle)*30, 15, math.sin(M.angle)*30}, {0,0,0}, {0,1,0})
    local proj = math_utils.perspective(math.rad(45), sw.extent.width/sw.extent.height, 0.1, 1000.0)
    ...
    -- Dispatch 10,000 Mesh Workgroups
    -- Each workgroup draws 32 blades (320,000 blades total)
