...
void main() {
    ...
    // Safety check for points behind near plane
    uint z = 0;
    if (viewZ > pc.z_near) {
    ...
    vec3 N = vec3(0.0, 0.0, 1.0); // Billboard normal
    vec3 V = normalize(-vViewPos);
    vec3 albedo = vColor.rgb;
    vec3 total_light = vec3(0.1) * albedo; // Ambient
    ...
    outColor = vec4(total_light, alpha);
}
