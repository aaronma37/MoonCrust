...
local M = {
	...
	pose_offset = 41, -- Aligned 40-byte offset for 52-byte local_position
	pose_is_double = false, -- Float precision as found in airport log
	lidar_offset = 16, -- User discovered 16 for airport log
	lidar_stride = 18, -- Livox CustomMsg standard stride (updated from 30)
	just_sought = false,
	bytes_processed = 0,
	throughput_mbs = 0,
...
function M.request_field_history(ch_id, offset, is_double)
	if not M.plot_history[ch_id] then
		M.plot_history[ch_id] = {}
	end
...
function M.load_mcap(path)
	M.init()
	if M.bridge then
		robot.lib.mcap_close(M.bridge)
		M.bridge = nil
	end
	for i = 1, M.MAX_TOPICS do
		M.message_buffers[i].active_ch = -1
	end -- Reset pool
	...
	for i = 0, count - 1 do
		if robot.lib.mcap_get_channel_info(M.bridge, i, info) then
			if info.topic ~= nil then
				...
				if t == M.lidar_topic then
					M.lidar_ch_id = info.id
				end
				if t:find("position") or t:find("pose") or t:find("odom") then
					M.pose_ch_id = info.id
				end
				...
				local ch = {
					id = info.id,
					topic = t,
					encoding = ffi.string(info.message_encoding or "u"),
					schema = ffi.string(info.schema_name or "u"),
					active = true,
					gtb_offset = nil,
					msg_size = msg_size,
				}
...
function M.auto_scan_pose()
	...
	local function check_offset(off, is_double)
		...
		if not buf or buf.size == 0 then
			return false
		end

		local x, y, z = 0, 0, 0
		if is_double then
			if off + 24 > buf.size then
				return false
			end
			local p = ffi.cast("double*", buf.data + off)
			x, y, z = p[0], p[1], p[2]
		else
			if off + 12 > buf.size then
				return false
			end
		...
		return math.abs(x) < 5000
			and math.abs(y) < 5000
			and math.abs(z) < 5000
			and (math.abs(x) > 0.0001 or math.abs(y) > 0.0001)
	end
	...
	if found then
		print(
			string.format(
				"Auto-Scan: SUCCESS! Found %s at offset %d",
				M.pose_is_double and "Double" or "Float",
				M.pose_offset
			)
		)
	else
...
function M.update(dt, raw_buffer)
	...
	for k in pairs(M._last_msg_ch) do
		M._last_msg_ch[k] = nil
	end
	for k in pairs(M._last_msg_offsets) do
		M._last_msg_offsets[k] = nil
	end
	...
	while (not M.paused or M.just_sought) and M.current_msg.log_time <= M.playback_time_ns do
		...
		if ch_id == M.pose_ch_id then
			...
			if M.pose_is_double and M.current_msg.data_size >= (M.pose_offset + 56) then
				...
				M.robot_pose.qx, M.robot_pose.qy, M.robot_pose.qz, M.robot_pose.qw =
					tonumber(p[3]), tonumber(p[4]), tonumber(p[5]), tonumber(p[6])
