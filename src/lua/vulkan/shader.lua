...
function M.compile_glsl(source, stage)
    ...
    local stage_flag = glslc_stage[ext] and ("-fshader-stage=" .. glslc_stage[ext]) or ""
    ...
    local cmd = string.format("glslc --target-env=vulkan1.3 %s %q -o %q", stage_flag, tmp_in, tmp_out)
